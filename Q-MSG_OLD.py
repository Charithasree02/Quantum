...
if page == "Compose":
    ...
    with st.form("tx_form"):
        ...
        # backend_options = [b.name for b in service.backends(min_num_qubits=4)] 
        # Explicitly get simulators and real backends
        ...
        if not backend_options: backend_options = ["ibmq_qasm_simulator"] # Fallback
        backend_name=c1_form.selectbox("Backend", backend_options, key="backend_select")
        K_ROUNDS = 2 
        shots=c2_form.number_input("Shots",value=20480,step=1024, key="shots_input") # Increased default shots for better quality
        use_test_img = st.checkbox("Use Test Image (16x16 red square)", value=False)
        submit_pressed = st.form_submit_button("Send")

        if submit_pressed:
            if (not imgf and not use_test_img) or not rc: st.warning("Please provide all inputs."); st.stop()
            code=rc.strip()
            # ... (Recipient check same) ...
            ...
            with st.spinner("Submitting quantum job..."):
                if use_test_img:
                    from PIL import Image
                ...
                # Pre-processing (Just to build circuits)
                ...
                if pal is None: raise ValueError("No palette") # Simple check
                # ... (We do full encoding to generate circuits) ...
                # Re-doing full encoding here just for circuit generation
                ...
                if "simulator" in backend_name:
                    st.info("Simulator detected. Auto-finalizing...")
                    ok, msg = process_and_finalize_tx(txid)
                    if ok: st.success("Sent and Processed! (Simulator)"); st.balloons()
                    else: st.error(msg)
                else:
                    st.success(f"Job Submitted! (ID: `{job_id}`). Go to 'Sent' tab to check status later.")
...
# --- Inbox Tab ---
    ...
    # Changed to show RUNNING as well
    data=supabase.table('transmissions').select("*,sender:sender_id(email)").eq('receiver_sharing_code',profile['sharing_code']).in_('status', ['COMPLETED', 'RUNNING']).order('created_at',desc=True).execute().data
    ...
    for it in data or []:
        ...
        with st.expander(f"From: {sender} on {created} [{status}]"):
            if status == "COMPLETED":
                ...
                st.info("Incoming Transmission... The quantum job is currently processing on IBM Hardware.")
                st.write(f"Job ID: `{it.get('ibm_job_id')}`")
                st.write("Click below to check if the job is done and download the result.")
                if st.button("Check Status & Finalize", key=f"check_inbox_{it['id']}"):
                     with st.spinner("Checking IBM Quantum..."):
                         ok, msg = process_and_finalize_tx(it['id'])
                         if ok: st.success(msg); st.rerun()
                         else: st.warning(msg)
            else:
                st.error(f"Transmission Failed or Unknown Status: {status}")
    ...
    for it in sents or []:
        with st.expander(f"To: `{it['receiver_sharing_code']}` | Status: {it['status']}"):
            if it['status']=="COMPLETED":
                ...
                if it.get("metrics"): st.json(it['metrics'])
            elif it['status']=="RUNNING":
                 st.info(f"Job `{it.get('ibm_job_id')}` is queued/running on IBM Quantum.")
                 if st.button("Check Status / Finalize", key=f"check_{it['id']}"):
                     with st.spinner("Checking IBM Quantum..."):
                         ok, msg = process_and_finalize_tx(it['id'])
                         if ok: st.success(msg); st.rerun()
                         else: st.warning(msg)
            else: st.write(f"Status: {it['status']}")
